...
def load_vgg(sess, vgg_path):
    ...
    # get the input,layer3,layer4,layer7 and keep_prob
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # 1x1 convolution of vgg layer7 output,use random normal to initialize the kernel,use L2 regularizer as the kernel regularizer
    ...
    # skip connection
    output_1 = tf.add(output,layer4_conv_1x1)
    
    # up-sampling
    output_2 = tf.layers.conv2d_transpose(output_1,num_classes,4,2,padding='same',
                                          kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                          kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # 1x1 convolution of layer3 output
    layer3_conv_1x1 = tf.layers.conv2d(vgg_layer3_out,num_classes,1,1,padding='same',
                              kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # skip connection
    output_3 = tf.add(output_2,layer3_conv_1x1)
    
    # up-sampling
    output_4 = tf.layers.conv2d_transpose(output_3,num_classes,16,8,padding='same',
                                           kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                           kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # reshape logits and labels to 2D tensor
...
def run():
    ...
    # Download pretrained vgg modelgit 
    ...
    # TF placeholder for correct label and learning rate
    ...
    with tf.Session() as sess:
        ...
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
                 correct_label, keep_prob, learning_rate)
        # save model
        save_path = saver.save(sess,'./savemodel/model')  
        print ("Model saved !")
